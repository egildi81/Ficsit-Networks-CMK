...
if not inet then error("FACTORY_CENTRAL EEPROM: pas d'InternetCard") end
...
if not ok or code ~= 200 or not body or body == "" then
    error("FACTORY_CENTRAL EEPROM: chargement échoué (HTTP "..tostring(code)..")")
end

local fn, err = load(body)
if not fn then error("FACTORY_CENTRAL EEPROM: parse — "..tostring(err)) end
