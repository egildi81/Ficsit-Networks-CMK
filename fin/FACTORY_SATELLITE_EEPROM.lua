...
if not inet then error("FACTORY_SAT EEPROM: pas d'InternetCard") end
...
if not ok or code ~= 200 or not body or body == "" then
    error("FACTORY_SAT EEPROM: chargement échoué (HTTP "..tostring(code)..")")
end

local fn, err = load(body)
if not fn then error("FACTORY_SAT EEPROM: parse — "..tostring(err)) end
