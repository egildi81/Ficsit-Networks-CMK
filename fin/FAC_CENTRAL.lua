-- FAC_CENTRAL.lua : agrégateur central du réseau de monitoring usine
-- Reçoit les données de tous les FAC_SATELLITE, agrège, push vers web.
-- Central aggregator for factory monitoring network.
-- Receives data from all FAC_SATELLITE, aggregates, pushes to web.
--
...
if not net  then error("FAC_CENTRAL: pas de NetworkCard") end
if not inet then error("FAC_CENTRAL: pas d'InternetCard") end
...
print("FAC_CENTRAL v"..VERSION)

-- print → GET_LOG (port 43 broadcast) / print → GET_LOG (port 43 broadcast)
print = function(...)
    local t={} for i=1,select('#',...)do t[i]=tostring(select(i,...))end
    pcall(function() net:broadcast(PORT_LOG,"FAC_CENTRAL",table.concat(t," ")) end)
end
print("=== FAC_CENTRAL v"..VERSION.." BOOT ===")
